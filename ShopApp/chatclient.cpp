...
//    connect(fileClient, SIGNAL(disconnected( )), fileClient, SLOT(deletelater( )));
    ...
    connect(connectButton, &QPushButton::clicked,
        ...
        if(connectButton->text() == tr("Log In")) {
            ...
            QMessageBox::information(this, "Chat in", "채팅방에 입장하셨습니다.");
            ...
            QMessageBox::information(this, "Chat Out", "채팅방을 나가게 됩니다. 대기실로 접속중...");
        }
    } );
...
void ChatClient::sendData(  )
{
    ...
    if(str.length( )) {
        ...
        /* 화면에 표시 : 앞에 '나'라고 추가 */
        message->append("<font color=orange>나</font> : " + str);
...
void ChatClient::slot_login(int login, int id)
{
    ...
    if(login == 1)   //login이 true이면 로그인 가능
    {
        ...
        QMessageBox::information(this, "Log In", "어서오세요 오스템입니다 *^^*");
    }
    else if(login == 2)
    {
